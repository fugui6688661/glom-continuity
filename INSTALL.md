...
收到配套的`.whl`文件后，可以安装到自己新建的工具环境。以下以`glom_continuity-0.1.0a3-py3-none-any.whl`为例；只有拿到该文件后才执行。
...
.continuity-tools/bin/python -m pip install --no-index --no-deps ./glom_continuity-0.1.0a3-py3-none-any.whl
...
.\.continuity-tools\Scripts\python.exe -m pip install --no-index --no-deps .\glom_continuity-0.1.0a3-py3-none-any.whl
