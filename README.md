...
开发分支版本：`0.1.0-alpha.7`，尚未发行。新加入的 Harness 自动恢复、原生命令和独立宿主启动器不在下方 Alpha.6 安装包中。[已验收范围与限制](docs/managed-host-validation.md)单独列出；普通对话自动保存仍未实现。下方安装说明只安装已公开的 Alpha.6，新代码不会覆盖旧附件。

给用 AI 助手持续做项目的人。把目标、限制、已定事项和下一步留在项目里，换会话后读回；引用文件变了，就先审阅变化。一个助手也能用。

目前公开下载的 v0.1.0-alpha.6 将现有 dev4 功能打包为预览版，不是稳定版。安装以对应 Release 的具名附件及配套清单为准；源码、安装包与实际调用的程序需分别核对版本。
